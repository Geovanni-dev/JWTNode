...
const createAdmin = async () => {
  // funcao assincrona para criar o ADM padrao, nao recebe requisicao
  try {
    const { name, email, password, role } = AdminSchema.parse({
      // valida os dados do ADM

      name: process.env.SEED_NAME || 'Admin',
      email: process.env.SEED_EMAIL || 'f5lOa@example.com',
    ...
    const admin = await prisma.user.upsert({
      // cria o ADM padrao no banco de dados usando prisma
      where: { email },
      update: {},
      create: {
