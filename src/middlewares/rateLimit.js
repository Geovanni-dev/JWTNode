...
const limiter = rateLimit({
  ...
  handler: (req, res) => {
    ...
    if (blockedIps[ip] && now < blockedIps[ip]) {
      // se o IP estiver bloqueado
      const secondsLeft = (blockedIps[ip] - now) / 1000; // tempo restante de bloqueio
      return res.status(429).json({
        error: `Ainda estamos bloqueando esse IP. Por favor, tente novamente em ${secondsLeft} segundos.`,
      });
    }
    ...
    return res.status(429).json({
      error:
        'Muitas tentativas frequentes de login. Por favor, tente novamente mais tarde.',
    });
  },
});
