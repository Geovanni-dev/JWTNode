...
const verifyCodeSchema = z.object({
  code: z
    ...
    .length(
      6,
      'o codigo de verificacao precisa conter exatamente 6 caracteres',
    ), // valida o codigo de verificacao do usuario, que deve conter exatamente 6 caracteres
  email: z.string().email('o email precisa ser valido'), // valida o email do usuario, que deve ser um email valido
});
...
const listUsersSchema = z.object({
  ...
  name: z
    .string()
    .min(3, 'o nome precisa conter no minimo 3 caracteres')
    .optional(), // valida o nome do usuario, que deve conter no minimo 3 caracteres, e torna o campo opcional
...
const createUser = async (req, res) => {
  try {
    ...
    const user = await prisma.user.create({
      // cria um novo usuario no banco de dados usando prisma
      data: {
    ...
    res.status(201).json({
      message:
        'Usuário criado com sucesso, verifique seu email para obter o código de verificação',
    }); // retorna uma resposta de sucesso caso o usuario seja criado com sucesso
  } catch (error) {
...
const verifyCode = async (req, res) => {
  try {
    ...
    const user = await prisma.user.findFirst({
      // busca o usuario no banco de dados usando prisma
      where: {
        email,
        verificationCode: code, // codigo de verificacao do usuario, definido pelo parametro code
      },
    ...
    await prisma.user.update({
      // atualiza o usuario no banco de dados usando prisma
      where: {
        id: user.id, // id do usuario, definido pelo parametro id
      },
      data: {
        verificationCode: null,
        verified: true, // atualiza o codigo de verificacao do usuario para null e o campo verified para true, indicando que o usuario foi verificado com sucesso
      },
    });
...
const login = async (req, res) => {
  try {
    ...
    if (!user.verified) {
      // se o usuario nao for verificado
      return res
        .status(401)
        .json({ error: 'Verifique seu email antes de fazer loguin' }); // retorna uma resposta de erro
    }
    ...
    const users = await prisma.user.findMany({
      ...
      where: {
        ...
        name: name
          ...
          : undefined, // contains serve pra buscas parciais
...
module.exports = {
  // exportando as funcoes do controller de usuario
  createUser,
  verifyCode,
  login,
  listAllUsers,
